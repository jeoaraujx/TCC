...
A análise comparativa realizada por \citeonline{Jung_2024}, fundamenta a escolha teórica do BERTopic para este trabalho, ao demonstrar que a arquitetura baseada em embeddings supera abordagens probabilísticas (como LDA) na captura de nuances semânticas em textos estruturados. No entanto, é crucial destacar uma distinção de escopo: enquanto \citeonline{Jung_2024} validaram a superioridade do modelo em corpora de textos longos (resumos e notícias), este estudo aplica a técnica em um cenário de maior desafio: a modelagem de títulos curtos. Neste contexto de alta esparsidade informacional, a literatura indica que métricas de coocorrência (como o NPMI utilizado por Jung) tendem a divergir, exigindo uma validação focada na coerência interpretativa ($C_v$) e na capacidade de agrupamento semântico, e não apenas na replicação direta dos indicadores estatísticos observados em textos longos.