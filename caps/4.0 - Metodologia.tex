...
\begin{figure}[H]
    ...
    \includegraphics[width=0.7\textwidth]{figs/DSRTrue.jpg} 
...
\begin{itemize}
    ...
    \item \textbf{Inspeção Visual dos \textit{Clusters} (Validade Semântica):} Para validar se os \textit{clusters} possuem coerência semântica interna (i.e., se estão \enquote{bem formados}) e se o mapa reflete uma estrutura de conhecimento válida, o especialista inspecionará o \gls{wizmap}. A validação consistirá em selecionar alguns \textit{clusters} e analisar os documentos que os compõem (através dos \textit{tooltips} de cada \textit{datapoint}). Se os documentos dentro de um mesmo \textit{cluster} forem coesos e pertencerem ao tema descrito pelo rótulo, o artefato será considerado válido para resolver o problema da exploração semântica.
\end{itemize}

\section{Apresentação dos Resultados e Comunicação}
\label{sec:comunicacao}

A etapa final do ciclo \gls{dsr}, conforme o \textit{framework} de \citeonline{Dresch_2015}, é a \enquote{Comunicação}. Esta fase é dedicada à documentação e disseminação dos achados obtidos durante o desenvolvimento e a avaliação do artefato.
