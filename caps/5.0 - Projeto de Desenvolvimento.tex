...
O desenvolvimento do artefato proposto neste trabalho segue uma abordagem estruturada e incremental, alinhada à metodologia \gls{dsr} detalhada no Capítulo \ref{chap:metodologia}. O objetivo é construir e validar um \textit{pipeline} computacional que venha a aprimorar e complementar a análise de publicações científicas na plataforma do \textbf{Observatório de dados públicos de ciência e tecnologia da Bahia}\footnote{Disponível em: \url{https://simcc.uesc.br/observatorio}}.

Conforme descrito na literatura \citeonline{Santos_2024, Jorge_2025}, o Observatório já possui uma arquitetura robusta para o mapeamento de competências, integrando fontes de dados heterogêneas como a Plataforma Lattes\footnote{Disponível em: \url{http://lattes.cnpq.br/}}, a Plataforma Sucupira\footnote{Disponível em: \url{https://sucupira.capes.gov.br/}}, o OpenAlex\footnote{Disponível em: \url{https://openalex.org/}} e o \gls{jcr}\footnote{Disponível em: \url{https://clarivate.com/webofsciencegroup/solutions/journal-citation-reports/}}. Seu sistema atual utiliza um processo de \gls{etl} com Apache Hop\footnote{Disponível em: \url{https://hop.apache.org/}} para consolidar as informações em um banco de dados PostgreSQL\footnote{Disponível em: \url{https://www.postgresql.org/}} e já emprega técnicas de \gls{pln} para a recuperação de informações baseadas em termos e palavras-chave.
...
Este capítulo detalha, portanto, a construção de um \textit{pipeline} que representa uma evolução para a arquitetura do Observatório. A solução proposta não visa substituir, mas sim enriquecer a funcionalidade de busca atual, introduzindo a modelagem de tópicos moderna com o \gls{bertopic}, o refinamento de rótulos com \gls{mmr} e a visualização interativa com o \gls{wizmap}. O foco é transcender a lista de resultados tradicional, permitindo que os usuários naveguem visualmente pelas principais áreas de pesquisa, identifiquem temas emergentes e compreendam as relações entre os diferentes campos do conhecimento de maneira orgânica.

\section[Tecnologias Utilizadas]{Tecnologias Utilizadas}
\label{sec:tecnologias_utilizadas}

O desenvolvimento do artefato proposto neste projeto assenta-se sobre a combinação de duas arquiteturas tecnológicas distintas: (1) a infraestrutura consolidada do \textbf{Observatório de dados públicos de ciência e tecnologia da Bahia}, que serve como fonte de dados e contexto de aplicação; e (2) o \textit{pipeline} de modelagem e visualização desenvolvido, que constitui o artefato central deste estudo.
...
Sobre a base conceitual do Observatório, este projeto implementa um novo \textit{pipeline} focado na descoberta e análise semântica do conhecimento. Dada a natureza deste trabalho como um estudo de caso (conforme metodologia \gls{dsr}), o \textit{pipeline} foi desenvolvido e validado em um ambiente de prototipagem computacional.
